...
const dataPath = __dirname + "/Ra thakur/approvedThreads.json";
const dataPending = __dirname + "raj thakur/pendingdThreads.json";
const fs = require("fs");
...
module.exports.run = async ({ event, api, args, Threads, handleReply, Users }) => {
  ...
  const cre = this.config.credits
      ...
      api.sendMessage(`[ OK ] Your group has been removed from the browsing list by the admin for the reason: ${lydo}`, idBox);
   	...
   	else api.sendMessage("[ OK ] ✨ApKa Group Approved Kar Diya Hai🙌.\n 🖤So Enjoy\n\n 💝🥀𝐎𝐖𝐍𝐄𝐑:- ☞raj thakur Raj xd☜ 💫\n 🖤𝚈𝚘𝚞 𝙲𝚊𝚗 𝙲𝚊𝚕𝚕 𝙷𝚒𝚖 〠 raj xd thakur 🖤\n😳𝐇𝐢𝐬 𝐅𝐚𝐜𝐞𝐛𝐨𝐨𝐤 𝐢𝐝🤓:- ☞👋For Any Kind Of Help Contact On Telegram  Username 👉 @Priyanshrajput😇", idBox, (error, info) => {
